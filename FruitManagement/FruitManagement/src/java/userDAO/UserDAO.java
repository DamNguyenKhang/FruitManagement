...
public class UserDAO implements IUserDAO {
    ...
    private static final String INSERT_USER = "INSERT INTO Users (userName, password, email, phone) VALUES (?, ?, ?, ?)";
    ...
    public boolean isDuplicateUserName(String userName){
        try(Connection con = DBConnection.getConnection()){
            PreparedStatement ptm = con.prepareStatement("SELECT * FROM Users WHERE userName = ?");
            ptm.setString(1, userName);
            ResultSet rs = ptm.executeQuery();
            if(rs.next()){
                return true;
